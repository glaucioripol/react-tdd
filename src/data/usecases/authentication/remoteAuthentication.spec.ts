...
describe('RemoteAuthentication', () => {
  ...
  test('Should throw UnexpectedError if HttPostClientReturn 400', async () => {
    const { sut, httpPostClientSpy } = makeSut()
    httpPostClientSpy.response = {
  ...
  test('Should throw InvalidCredentialsError if HttPostClientReturn 404', async () => {
    const { sut, httpPostClientSpy } = makeSut()
    httpPostClientSpy.response = {
  ...
  test('Should throw InvalidCredentialsError if HttPostClientReturn 500', async () => {
    const { sut, httpPostClientSpy } = makeSut()
    httpPostClientSpy.response = {
